...
function roadmap(){
  const P = [
    ...
    { n:'12', t:'CMS-native serving (plumbing done, editing NOT)', s:'progress', d:'What IS proven: every page is served live from Directus and a zero-trust gate confirms it (a sentinel written through the CMS surfaces in the re-served HTML). What is NOT done and was over-claimed: it is one shared "pages" collection holding a JSON blob — a client logging in sees opaque JSON, not "Products / Orders / Menu". A real per-site editing experience is production-quality work still ahead (see PQ3).' },
    ...
    { n:'PQ', t:'PRODUCTION QUALITY — the real work before anything is "sold"', s:'progress', d:'Honest reset (2026-07-02): M1–M5 proved plumbing (pipeline, forms, migrations, auth, alerts) but the OUTPUT is not agency-grade — every site is one template recolored, ecommerce cannot actually sell, and the CMS is a JSON blob. No monetization until a demanding creative director would ship the output. The market/pricing milestone is dropped. The bar below is measured on real produced sites, not mechanics.' },
    { n:'PQ1', t:'Distinct design per brief', s:'progress', d:'System now has a LAYOUT dimension separate from colour/font: four hand-built hero treatments (full-bleed / split / centered / editorial), two nav styles, and section banding — chosen deterministically from the brief, one per project. Proven: a law firm and a wedding photographer on the SAME design language render as structurally different sites (centered photoless hero + centered nav vs split hero + standard nav). Gated by layout:check (24 assertions). Still ahead: more hero/section variants and deeper per-section layout variety so the range keeps widening.' },
    { n:'PQ2', t:'Ecommerce that actually sells', s:'progress', d:'A store brief now ships a real shop: a product grid loaded from the live database with Add-to-cart, a cart page (quantities, remove, total), and a checkout that writes a REAL order — priced on the server from database prices (a tampered cart cannot change what is charged), line items with price snapshots, all in one transaction. The browser reviewer BUYS on every store build and the order must land. Proven live: a ceramics store built, a 3-item order placed through the public site, $141 total computed server-side. Gated by ecom:check (20 assertions). Ahead: product detail pages, options/variants, stock.' },
    { n:'PQ3', t:'A CMS a client can actually use', s:'progress', d:'Every site now has a Content tab: the owner sees their REAL collections (Products, Menu, Categories, Posts…) and edits, adds or deletes records — changes go live on the site instantly. Proven live: edited a product\'s price on a ceramics store and the live shop tracked it. Owner-only (a two-user gate proves nobody else can touch it). Note: this edits the app database directly through Relay — Directus (a separate database) can\'t reach it, so Relay owns the editing surface; Directus keeps serving the presentation pages. Gated by content:check (19). Ahead: image upload + richer field types.' },
    { n:'PQ4', t:'Then — free self-serve (was M6, de-scoped from paid)', s:'next', d:'Only after PQ1–PQ3: anyone signs up and builds for free (it is free — no Stripe). Sign-in + ownership already shipped in M4; this is just opening the door once the product is worth walking through.' },
  ];
