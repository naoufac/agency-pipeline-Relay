...
cd /srv/relay 2>/dev/null || cd /root/agency-pipeline
set -a; . ./.env; set +a
...
[ -s "$KEY" ] || { echo "backup key missing at $KEY"; exit 1; }
...
[ "$DUMP_BYTES" -gt 1000000 ] || { echo "dump suspiciously small ($DUMP_BYTES B) — refusing to ship it"; exit 1; }
...
grep -q "projects" "$OUT/verify.list" || { echo "restore-list has no projects table"; exit 1; }
...
tar -tzf "$OUT/verify.tar.gz" | grep -q "relay-android.keystore" || { echo "secrets tar is missing the keystore"; exit 1; }
