...
function __searchbox(el,count){if(count<8||el.__hasSearch||!el.parentNode)return;el.__hasSearch=1;var w=document.createElement('input');w.type='search';w.placeholder='Search…';w.className='grid-search';w.setAttribute('aria-label','Search this list');w.oninput=function(){var q=w.value.toLowerCase().trim();Array.prototype.forEach.call(el.children,function(c){c.style.display=!q||(c.textContent||'').toLowerCase().indexOf(q)>=0?'':'none'})};el.parentNode.insertBefore(w,el)}
