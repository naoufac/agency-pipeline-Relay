...
Array.prototype.forEach.call(document.querySelectorAll('.products[data-products]'),function(el){var tbl=el.getAttribute('data-products');fetch('/api/site/'+encodeURIComponent(pid)+'/data/'+encodeURIComponent(tbl)).then(function(r){return r.json()}).then(function(d){var rows=((d&&d.rows)||[]).filter(function(o){return o&&o.id!=null});if(!rows.length)return;el.innerHTML='';var IMG=/image|photo|picture|thumb/i;rows.slice(0,24).forEach(function(o){var card=document.createElement('div');card.className='card';var keys=Object.keys(o);var dhref=(tbl==='products'&&/^\\d+$/.test(String(o.id)))?('product-'+o.id+'.html'):null;var imgk=keys.filter(function(k){return IMG.test(k)&&typeof o[k]==='string'&&(/^https?:/.test(o[k])||o[k].charAt(0)==='/')})[0];if(imgk){var im=document.createElement('img');im.src=o[imgk];im.alt='';im.loading='lazy';im.style.cssText='width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:8px;margin-bottom:12px';im.onerror=function(){im.remove()};if(dhref){var la=document.createElement('a');la.className='p-imglink';la.href=dhref;la.appendChild(im);card.appendChild(la)}else card.appendChild(im)}var title=String(o.title||o.name||('#'+o.id));var h=document.createElement('h3');if(dhref){var ta=document.createElement('a');ta.href=dhref;ta.textContent=title;h.appendChild(ta)}else h.textContent=title;card.appendChild(h);var pk=keys.filter(function(k){return /^(price|amount|cost)$/.test(k)})[0];var price=pk?parseFloat(o[pk]):NaN;if(isFinite(price)){var pr=document.createElement('div');pr.className='p-price';pr.textContent='$'+price.toFixed(2);card.appendChild(pr)}var desc=String(o.description||o.body||'').slice(0,120);if(desc){var dp=document.createElement('p');dp.textContent=desc;card.appendChild(dp)}if(isFinite(price)){var add=document.createElement('button');add.type='button';add.className='btn p-add';add.textContent='Add to cart';add.onclick=function(){window.relayCartAdd({id:o.id,title:title,price:price},add)};card.appendChild(add)}el.appendChild(card)})}).catch(function(){})});
__cartRender();
