...
function __rcards(el,items){if(!items||!items.length)return;el.innerHTML='';var IMG=/image|photo|avatar|cover|logo|thumb|picture|banner/i,MONEY=/price|amount|cost|total|fee|budget|salary|rate/i;var NOISE=function(s){return /^[a-z0-9]+(?:[-_][a-z0-9]+)+$/.test(s)||/^#?\\d+(\\.\\d+)?$/.test(s)};items.slice(0,12).forEach(function(o){o=o||{};var keys=Object.keys(o).filter(function(k){return['id','created_at','password_hash','password'].indexOf(k)<0&&o[k]!=null&&o[k]!==''});var tk=['name','title','business','listing','label'].filter(function(k){return o[k]})[0]||keys.filter(function(k){return !NOISE(String(o[k]))})[0]||keys[0];var card=document.createElement('div');card.className='card';var imgk=keys.filter(function(k){return IMG.test(k)&&typeof o[k]==='string'&&(/^https?:/.test(o[k])||o[k].charAt(0)==='/')})[0];if(imgk){var im=document.createElement('img');im.src=o[imgk];im.alt='';im.loading='lazy';im.style.cssText='width:100%;aspect-ratio:16/10;object-fit:cover;border-radius:var(--radius);margin-bottom:12px';im.onerror=function(){im.remove()};card.appendChild(im);card.classList.add('has-img');}if(tk){var h=document.createElement('h3');h.textContent=String(o[tk]);card.appendChild(h);}keys.filter(function(k){return k!==tk&&k!==imgk}).slice(0,6).forEach(function(k){var v=o[k],p=document.createElement('p');if(typeof v==='boolean'){if(!v||/active|enabled|visible|published/.test(k))return;p.textContent='✓ '+k.replace(/_/g,' ');}else if(MONEY.test(k)&&!isNaN(parseFloat(v))){p.textContent='$'+parseFloat(v).toFixed(2);p.style.fontWeight='600';}else{var s=String(v);if(NOISE(s))return;p.textContent=s.slice(0,180);}card.appendChild(p);});el.appendChild(card);});}
...
window.relayCartAdd=function(p,btn){var c=__cart();var f=null;for(var i=0;i<c.length;i++)if(c[i].id===p.id)f=c[i];if(f)f.qty=Math.min(99,f.qty+1);else c.push({id:p.id,title:p.title,price:p.price,qty:1});__cartSave(c);if(btn){var t=btn.textContent;btn.textContent='Added ✓';setTimeout(function(){btn.textContent=t},1200)}};
