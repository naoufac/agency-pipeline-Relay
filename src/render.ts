...
/* feed loader: fill each .feed[data-feed] with the site's own live submissions (read path). No-ops under file:// (gate), populates over HTTP. textContent only — submitted data is never HTML. */
(function(){var pid=window.RELAY_PID;if(!pid)return;var feeds=document.querySelectorAll('.feed[data-feed]');if(!feeds.length)return;Array.prototype.forEach.call(feeds,function(el){var form=el.getAttribute('data-feed');fetch('/api/submissions?id='+encodeURIComponent(pid)+'&form='+encodeURIComponent(form)).then(function(r){return r.json()}).then(function(d){var rows=(d&&d.submissions)||[];if(!rows.length)return;el.innerHTML='';rows.slice(0,12).forEach(function(s){var data=s.data||{},keys=Object.keys(data);var tk=['name','title','business','listing'].filter(function(k){return data[k]})[0]||keys[0];var card=document.createElement('div');card.className='card';if(tk){var h=document.createElement('h3');h.textContent=String(data[tk]);card.appendChild(h);}keys.filter(function(k){return k!==tk}).slice(0,4).forEach(function(k){var p=document.createElement('p');p.textContent=String(data[k]);card.appendChild(p);});el.appendChild(card);});}).catch(function(){});});})();</script>
</body></html>`;
}
