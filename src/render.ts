...
export function renderPage(spec: any, ctx: { pages: any[]; slug: string; title: string; projectId?: string; theme?: string; layout?: Layout; forms?: Record<string, any[]>; primaryTable?: string; formSlug?: string; accountLinks?: boolean; locale?: string; siteBase?: string; localBusiness?: boolean; bizType?: string; bizFacts?: any }): string {
  ...
  // A class studio is not a store. Cart/checkout JS only ships when this page actually sells.
  const wantsCart = ((spec && spec.sections) || []).some((s: any) => s && /^(products|product|cart|checkout)$/.test(String(s.type)));
...
${wantsCart ? `/* PQ2: shop grid — real products from the DB, each with Add-to-cart (client cart; order priced server-side).
...
${wantsCart ? `/* PQ2: client cart (localStorage, per site). Prices here are DISPLAY ONLY — the server recomputes the
...
<!-- ARC J: cookieless first-party analytics beacon. Fires ONCE per page view via
     navigator.sendBeacon (fallback: fetch keepalive). Same-origin /api/hit only —
     no external service, no cookie, no localStorage, no fingerprinting. -->
<script>(function(){try{var p=location.pathname;var b=JSON.stringify({p:p});if(navigator.sendBeacon){navigator.sendBeacon('/api/hit',b)}else{fetch('/api/hit',{method:'POST',body:b,keepalive:true,headers:{'content-type':'application/json'}}).catch(function(){})}}catch(e){}})();</script>
</body></html>`;
