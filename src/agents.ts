...
const ROLE: Record<string, string> = {
  ...
  build:       'You are the Build department building ONE page of a multi-page site (named in "YOU ARE BUILDING THIS PAGE"). You receive brand tokens (JSON palette hex) and copy (JSON). Output a COMPLETE self-contained HTML document for THIS page only (<!doctype html> ... </html>), with a shared STICKY top nav linking all listed pages via relative <slug>.html (current page highlighted). ' +
               ...
               'Design to a HIGH 2024 bar, not basic: a glass sticky nav (backdrop-blur bg-white/70 border-b) that is MOBILE-RESPONSIVE — on a ≤640px phone the links must wrap or collapse (a tiny inline <script> hamburger toggle is allowed), and must NEVER overflow or get cut off; keep the link row from overflowing with flex-wrap; a bold hero with class="font-display" headings at text-5xl md:text-7xl tracking-tight and a gradient accent (bg-gradient-to-r from-[#a] to-[#b] bg-clip-text text-transparent); generous rhythm (sections py-24 md:py-32, max-w-6xl mx-auto px-6); cards rounded-2xl ring-1 ring-black/5 shadow-sm hover:shadow-md transition; a real multi-column footer. Headings use class="font-display" (or class="font-serif-display" for an editorial/luxury brief). ' +
               'PHOTOGRAPHY (REQUIRED — every page needs real photos): open with a hero that has a REAL photo — a full-bleed background <img data-q="..." class="absolute inset-0 -z-10 w-full h-full object-cover"> behind the headline (add a dark overlay for contrast), or a large framed side image — and add at least 2 more photos in feature/gallery/about/team sections (minimum 3 photos total). Use <img src="" data-q="2-4 word stock-photo search" alt="..." class="..."> — the build system swaps each for a REAL licensed photo served locally. Precise on-brief queries (e.g. data-q="artisan sourdough bread", data-q="smiling baker portrait"). Size with Tailwind (object-cover, rounded-2xl, aspect-video / h-full). NEVER hardcode an external image URL (they 404), and NEVER add <script src=...> or the Tailwind CDN (cdn.tailwindcss.com) — Tailwind is compiled & inlined for you, so the page must be 100% self-contained (no external scripts, styles, or fonts). Use CSS gradients/shapes/inline SVG for abstract or brand visuals. Use the EXACT copy provided, but REPLACE any bracketed placeholder (e.g. [Studio Name], [Class Name]) with a realistic specific value — never ship brackets or lorem ipsum. Output ONLY raw HTML — no markdown, no fences, no commentary.',
