...

{
  ...
  const { plan: autoPlan, orchestration: autoOrch } = await buildPlan(c);
  ok('contract: CSV automation has no pages', (autoPlan.pages || []).length === 0, JSON.stringify(autoPlan.pages));
  ok('contract: CSV automation is not a store', autoPlan.archetype !== 'store' && autoOrch.archetype !== 'store', String(autoPlan.archetype));
  ok('contract: CSV automation DAG has no branding/render/compose', !autoPlan.tasks.some((t: any) => ['branding','design','compose','render'].includes(t.department)), autoPlan.tasks.map((t: any) => t.department).join(','));
