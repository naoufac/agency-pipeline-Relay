...
export const PRIVATE_READ = /^(orders?|order_items|bookings?|appointments?|reservations?|reservation_requests?|submissions?|messages?|enquir(?:y|ies)|inquir(?:y|ies)|leads?|registrations?|signups?|sign_ups?|rsvps?|applications?|waitlists?|waivers?|customers?|clients?|members?|patients?|guests?|attendees?|preorders?|pre_orders?|deliveries|shipments?|tracking_events?|users?|accounts?|sessions?|tokens?|payments?|invoices?|subscriptions?|donations?)$/i;
