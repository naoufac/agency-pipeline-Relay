...
export const STRINGS: Record<string, Record<Locale, string>> = {
  ...
  chain_rules_h:     { en: 'The rules it enforces', it: 'Le regole che applica', fr: 'Les règles qu’il applique', es: 'Las reglas que aplica', de: 'Die Regeln, die sie durchsetzt' },
  chain_rules_lead:  { en: 'Business policies, decided for this business and enforced by the system on every booking — not suggestions.', it: 'Regole di business, decise per questa attività e applicate dal sistema a ogni prenotazione — non suggerimenti.', fr: 'Des règles métier, décidées pour cette activité et appliquées par le système à chaque réservation — pas des suggestions.', es: 'Reglas de negocio, decididas para este negocio y aplicadas por el sistema en cada reserva — no sugerencias.', de: 'Geschäftsregeln, für dieses Geschäft festgelegt und vom System bei jeder Buchung durchgesetzt — keine Vorschläge.' },
  chain_rule_notice: { en: 'Bookings need at least {n}h notice', it: 'Le prenotazioni richiedono almeno {n} ore di anticipo', fr: 'Les réservations demandent au moins {n} h de préavis', es: 'Las reservas requieren al menos {n} h de antelación', de: 'Buchungen brauchen mindestens {n} Std. Vorlauf' },
  chain_rule_cancel: { en: 'Free cancellation up to {n}h before', it: 'Cancellazione gratuita fino a {n} ore prima', fr: 'Annulation gratuite jusqu’à {n} h avant', es: 'Cancelación gratuita hasta {n} h antes', de: 'Kostenlose Stornierung bis {n} Std. vorher' },
  chain_rule_capacity:{ en: 'Up to {n} booking(s) per time slot', it: 'Fino a {n} prenotazioni per fascia oraria', fr: 'Jusqu’à {n} réservation(s) par créneau', es: 'Hasta {n} reserva(s) por franja horaria', de: 'Bis zu {n} Buchung(en) pro Zeitfenster' },
  ...
  err_too_soon:     { en: 'that time is too soon — book at least {n}h ahead', it: 'quell’orario è troppo a ridosso — prenota con almeno {n} ore di anticipo', fr: 'ce créneau est trop proche — réservez au moins {n} h à l’avance', es: 'esa hora está demasiado cerca — reserva con al menos {n} h de antelación', de: 'dieser Termin ist zu kurzfristig — bitte mindestens {n} Std. im Voraus buchen' },
  mail_confirm_subject: { en: 'Your {x} is confirmed received', it: 'La tua {x} è stata ricevuta', fr: 'Votre {x} a bien été reçue', es: 'Tu {x} ha sido recibida', de: 'Deine {x} ist eingegangen' },
  mail_confirm_body: { en: 'We received your {x}. Your receipt (status updates live here):\n\n{link}\n\nKeep this link — it is your reference.', it: 'Abbiamo ricevuto la tua {x}. La tua ricevuta (lo stato si aggiorna qui):\n\n{link}\n\nConserva questo link — è il tuo riferimento.', fr: 'Nous avons bien reçu votre {x}. Votre reçu (le statut se met à jour ici) :\n\n{link}\n\nConservez ce lien — c’est votre référence.', es: 'Hemos recibido tu {x}. Tu recibo (el estado se actualiza aquí):\n\n{link}\n\nGuarda este enlace — es tu referencia.', de: 'Wir haben deine {x} erhalten. Dein Beleg (der Status aktualisiert sich hier):\n\n{link}\n\nBewahre diesen Link auf — er ist deine Referenz.' },
  ...
  // lifecycle loop — the visitor hears the owner's verdict, and gets a nudge before their slot
  mail_status_confirmed_subject: { en: 'Your {x} is confirmed ✓', it: 'La tua {x} è confermata ✓', fr: 'Votre {x} est confirmée ✓', es: 'Tu {x} está confirmada ✓', de: 'Deine {x} ist bestätigt ✓' },
  mail_status_confirmed_body: { en: 'Good news — your {x} has been confirmed. Your receipt:\n\n{link}', it: 'Buone notizie — la tua {x} è stata confermata. La tua ricevuta:\n\n{link}', fr: 'Bonne nouvelle — votre {x} a été confirmée. Votre reçu :\n\n{link}', es: 'Buenas noticias — tu {x} ha sido confirmada. Tu recibo:\n\n{link}', de: 'Gute Nachricht — deine {x} wurde bestätigt. Dein Beleg:\n\n{link}' },
  mail_status_declined_subject: { en: 'About your {x}', it: 'Riguardo alla tua {x}', fr: 'À propos de votre {x}', es: 'Sobre tu {x}', de: 'Zu deiner {x}' },
  mail_status_declined_body: { en: 'Unfortunately your {x} could not be accommodated this time. Details:\n\n{link}\n\nYou are welcome to pick another time.', it: 'Purtroppo questa volta non è stato possibile accettare la tua {x}. Dettagli:\n\n{link}\n\nPuoi scegliere un altro orario quando vuoi.', fr: 'Malheureusement, votre {x} n’a pas pu être acceptée cette fois. Détails :\n\n{link}\n\nVous pouvez choisir un autre horaire.', es: 'Lamentablemente tu {x} no pudo ser aceptada esta vez. Detalles:\n\n{link}\n\nPuedes elegir otro horario cuando quieras.', de: 'Leider konnte deine {x} diesmal nicht angenommen werden. Details:\n\n{link}\n\nDu kannst gerne eine andere Zeit wählen.' },
  mail_status_cancelled_subject: { en: 'Your {x} has been cancelled', it: 'La tua {x} è stata annullata', fr: 'Votre {x} a été annulée', es: 'Tu {x} ha sido cancelada', de: 'Deine {x} wurde storniert' },
  mail_status_cancelled_body: { en: 'Your {x} has been cancelled. Details:\n\n{link}\n\nYou are welcome to book again anytime.', it: 'La tua {x} è stata annullata. Dettagli:\n\n{link}\n\nPuoi prenotare di nuovo quando vuoi.', fr: 'Votre {x} a été annulée. Détails :\n\n{link}\n\nVous pouvez réserver à nouveau à tout moment.', es: 'Tu {x} ha sido cancelada. Detalles:\n\n{link}\n\nPuedes reservar de nuevo cuando quieras.', de: 'Deine {x} wurde storniert. Details:\n\n{link}\n\nDu kannst jederzeit erneut buchen.' },
  mail_reminder_subject: { en: 'Reminder: your {x} is coming up', it: 'Promemoria: la tua {x} si avvicina', fr: 'Rappel : votre {x} approche', es: 'Recordatorio: tu {x} se acerca', de: 'Erinnerung: deine {x} steht bevor' },
  mail_reminder_body: { en: 'A friendly reminder — your {x} is scheduled for {when}.', it: 'Un gentile promemoria — la tua {x} è prevista per il {when}.', fr: 'Petit rappel — votre {x} est prévue le {when}.', es: 'Un recordatorio — tu {x} está programada para el {when}.', de: 'Eine freundliche Erinnerung — deine {x} ist für {when} geplant.' },
  act_confirm_btn: { en: 'Confirm this {x}', it: 'Conferma questa {x}', fr: 'Confirmer cette {x}', es: 'Confirmar esta {x}', de: 'Diese {x} bestätigen' },
  act_decline_btn: { en: 'Decline', it: 'Rifiuta', fr: 'Refuser', es: 'Rechazar', de: 'Ablehnen' },
  act_done_confirmed: { en: 'Confirmed — the customer has been notified by email.', it: 'Confermata — il cliente è stato avvisato via email.', fr: 'Confirmée — le client a été prévenu par e-mail.', es: 'Confirmada — el cliente ha sido avisado por correo.', de: 'Bestätigt — der Kunde wurde per E-Mail benachrichtigt.' },
