...
try {
  ...
  ok('server: the act page escapes the visitor name (no visitor→owner XSS)', /const who = esc\(String\(/.test(serverSrc) && /import \{[^}]*\besc\b[^}]*\} from '\.\/components\.ts'/.test(serverSrc));
